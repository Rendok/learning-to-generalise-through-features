...
def generate_random_data(length):
    ...
    states = np.random.randn(length, 84, 84, 3)  # (W, H, C)
    actions = np.random.randn(length, 4, 1)
    labels = np.random.randn(length, 840, 84, 3)  # (W, H, C)
...
def generate_data(batch_size, planning_horizon):
    ...
    if DEBUG:
        ...
        print('inputs:', inputs[-2:, 0, 0, 0] / 255., 'actions:', actions[-2:, 0], 'labels:', labels[-2:, 0, 0, 0] / 255.)

    return inputs / 255., actions, labels / 255.
...
def generate(n_batches, batch_size, planning_horizon, path):
    ...
    with h5py.File(path, 'w') as f:
        d_states = f.create_dataset('states', (data_size, 84, 84, 3), compression="gzip")  # dtype='i4'
        d_actions = f.create_dataset('actions', (data_size, 4), compression="gzip")
        d_labels = f.create_dataset('labels', (data_size, 84, 84, 3), compression="gzip")
...
if __name__ == "__main__":
    ...
    generate(n_batches=5, batch_size=64, planning_horizon=20, path=path_tr)
    generate(n_batches=2, batch_size=64, planning_horizon=20, path=path_val)
