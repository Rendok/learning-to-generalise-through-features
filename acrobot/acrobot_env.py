...
class AcrobotEnv(py_environment.PyEnvironment):

    """
    Classic Acrobot task extended to a continuous actions space. Learning is made from raw pixels.
    ...
    **ACTIONS:**
    The action is in [-1, 1] torque on the joint between
    ...
    **REFERENCE:**
    ...
    def _reset(self):
        ...
        return ts.restart(self._get_ob())
    ...
    def _step(self, a):
        ...
        if terminal:
            return ts.termination(self._get_ob(), reward)
        else:
            return ts.transition(self._get_ob(), reward=reward, discount=1.0)
