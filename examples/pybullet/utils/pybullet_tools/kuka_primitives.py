...
def get_ik_fn(robot, fixed=[], teleport=False, num_attempts=10):
    import gym_kuka_multi_blocks.envs.kuka_utils as unt
    ...
    def fn(body, pose, grasp):
        ...
        for _ in range(num_attempts):
            ...
            q_approach = unt.inverse_kinematics(robot, grasp.link, approach_pose)
            if (q_approach is None) or any(pairwise_collision(robot, b) for b in obstacles):
                continue
            conf = BodyConf(robot, q_approach)
            q_grasp = unt.inverse_kinematics(robot, grasp.link, gripper_pose)
            if (q_grasp is None) or any(pairwise_collision(robot, b) for b in obstacles):
                print(any(pairwise_collision(robot, b) for b in obstacles))
                continue
            if teleport:
